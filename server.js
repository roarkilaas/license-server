    ...
    static async processValidation(licenseKey, machineId, clientInfo = {}) {
        try {
            ...
            return {
                ...
                license_type: license.license_type || 'machine',  // Include license_type
...
// Network/Floating license validation endpoint
app.post('/api/license/checkout', async (req, res) => {
    const { license_key, machine_id, session_id, client_info } = req.body;
    ...
    try {
        ...
        const validation = LicenseValidator.validateLicense(license);
        if (!validation.valid) {
            return res.json({
                valid: false,
                reason: validation.reason,
        ...
        // Use license_type field instead of checking features
        const isNetworkLicense = (license.license_type === 'network');
        
        if (isNetworkLicense) {
            // Network license logic
            return await handleNetworkLicense(license, hashedMachineId, session_id || generateId(), client_info, res);
        } else {
            // Traditional machine-based license
            return await handleMachineLicense(license, hashedMachineId, client_info, res);
        }
...
async function handleNetworkLicense(license, machineId, sessionId, clientInfo, res) {
    const now = new Date().toISOString();
    const heartbeatTimeout = 60; // seconds
    
    // Clean up expired sessions (no heartbeat for > heartbeatTimeout)
    await db.run(`
        DELETE FROM activations 
        WHERE license_id = ? 
        AND datetime(last_heartbeat, '+${heartbeatTimeout} seconds') < datetime('now')
    `, [license.id]);
    ...
    // Check if this machine/session already has a checkout
    const existingSession = await db.get(`
        SELECT * FROM activations 
        WHERE license_id = ? AND (machine_fingerprint = ? OR id = ?)
    `, [license.id, machineId, sessionId]);
    ...
    if (existingSession) {
        ...
        await db.run(
            'UPDATE activations SET last_heartbeat = ?, client_info = ? WHERE id = ?',
            [now, JSON.stringify(clientInfo), existingSession.id]
        );
        
        res.json({
            valid: true,
            license_type: 'network',  // Always return actual license_type
            license_key: license.license_key,
            product_id: license.product_id,
            session_id: existingSession.id,
            heartbeat_interval: heartbeatTimeout / 2,
            expiry: Math.floor(new Date(license.expiry_date).getTime() / 1000),
            features: features,
            concurrent_users: activeCount.count,
            max_concurrent: license.max_activations
        });
    } else {
        ...
        if (activeCount.count >= license.max_activations) {
            res.json({
                valid: false,
                reason: 'All network licenses in use',
                code: 'NETWORK_LICENSES_EXHAUSTED',
                current_users: activeCount.count,
                max_concurrent: license.max_activations
            });
        } else {
            // Create new session
            ...
            `, [sessionId, license.id, machineId, now, now, JSON.stringify(clientInfo)]);
            ...
            res.json({
                ...
                expiry: Math.floor(new Date(license.expiry_date).getTime() / 1000),
                features: features,
                concurrent_users: activeCount.count + 1,
                max_concurrent: license.max_activations
            });
        }
    }
...
async function handleMachineLicense(license, machineId, clientInfo, res) {
    // Use existing traditional license validation logic but ensure license_type is returned
    ...
    // Ensure license_type is always included in response
    if (result.valid) {
        result.license_type = license.license_type || 'machine';
    }
    
    res.json(result);
}

// Heartbeat endpoint for network licenses
...
// Release/checkin network license
app.post('/api/license/checkin', async (req, res) => {
    ...
    try {
        const result = await db.run(
            'DELETE FROM activations WHERE id = ?',
            [session_id]
        );
        
        if (result.changes > 0) {
            res.json({ success: true, message: 'License checked in successfully' });
        } else {
...
app.post('/api/license/validate', async (req, res) => {
    ...
    try {
        ...
        // If this is a network license, redirect to checkout endpoint logic
        if (license.license_type === 'network') {
            return res.json({
                valid: false,
                reason: 'Network license requires checkout endpoint',
                code: 'NETWORK_LICENSE_USE_CHECKOUT',
                license_type: 'network'
            });
        }

        // Process as machine license
        const result = await LicenseValidator.processValidation(
            license_key, 
            machine_id, 
            client_info || {}
        ...
        // Ensure license_type is included
        if (result.valid) {
            result.license_type = license.license_type || 'machine';
...
app.post('/api/license/create', async (req, res) => {
    ...
    try {
        const license = await LicenseDatabase.createLicense({
            ...
            features: features || [],
            license_type: license_type || 'machine' 
        });
...
app.post('/api/admin/customers', async (req, res) => {
    ...
    try {
        await db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `, [id, name, email, company || null, phone || null, address || null]);
...
app.post('/api/admin/products', async (req, res) => {
    const { id, name, description, version, features, default_max_activations, default_validity_days } = req.body;
...
app.put('/api/admin/products/:id', async (req, res) => {
    ...
    try {
        ...
        await db.run(`UPDATE products SET ${fields} WHERE id = ?`, values);
...
app.delete('/api/admin/products/:id', async (req, res) => {
    ...
    try {
        ...
        const licenseCount = await db.get(
        ...
        if (licenseCount.count > 0) {
            return res.status(400).json({ 
                error: `Cannot delete product: ${licenseCount.count} license(s) still reference this product` 
            });
        }
...
app.delete('/api/admin/customers/:id', async (req, res) => {
    ...
    try {
        // Check if customer has any licenses
        const licenseCount = await db.get(
        ...
        if (licenseCount.count > 0) {
            return res.status(400).json({ 
                error: `Cannot delete customer: ${licenseCount.count} license(s) still reference this customer` 
            });
        }
...
app.get('/api/admin/licenses', async (req, res) => {
    try {
        const { status, customer_id, product_id, limit = 100 } = req.query;
...
app.put('/api/admin/licenses/:license_key', async (req, res) => {
    const { license_key } = req.params;
    const updates = req.body;
    ...
    try {
        ...
        await db.run(
            `UPDATE licenses SET ${fields}, updated_at = CURRENT_TIMESTAMP WHERE license_key = ?`,
            values
        );
        ...
        // Log the update
        await LicenseDatabase.logAudit(license.id, 'updated', null, updates);
...
// Delete license
app.delete('/api/admin/licenses/:license_key', async (req, res) => {
    const { license_key } = req.params;
    ...
    try {
        ...
        await db.run('DELETE FROM activations WHERE license_id = ?', [license.id]);
        
        // Delete audit log entries for this license
        await db.run('DELETE FROM license_audit_log WHERE license_id = ?', [license.id]);
        
        // Delete the license
        const result = await db.run('DELETE FROM licenses WHERE id = ?', [license.id]);
...
app.get('/api/admin/reports/:type', async (req, res) => {
    ...
    try {
        if (type === 'summary') {
            ...
            const expiredLicenses = await db.get('SELECT COUNT(*) as count FROM licenses WHERE expiry_date < datetime("now")');
            const totalActivations = await db.get('SELECT COUNT(*) as count FROM activations');
            const uniqueCustomers = await db.get('SELECT COUNT(DISTINCT customer_id) as count FROM licenses');
            ...
            res.json({
                ...
                total_activations: totalActivations.count,
...
app.get('/api/admin/audit', async (req, res) => {
    try {
        const { license_key, limit = 100 } = req.query;
